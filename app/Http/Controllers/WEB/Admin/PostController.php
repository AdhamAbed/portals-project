...
use App\Models\Post as TargetModel;
...
class PostController extends Controller
{
    ...
    {
        $this->baseFolder = 'admin.posts';
        $this->indexRoute = 'admin.posts.index';
    ...
    public function store(Request $request)
    {
        ...
        $item->publication_time = $request->publication_time;
        $item->reference_link = $request->reference_link;
        ...
        if(isset($request->image)){
            $item->image = uploadImage($request->image, 'posts');    
        }
        
        if(isset($request->video)){
            $item->video = uploadFile($request->video, 'posts');    
        }
        ...
        if($request->has('filename')  && !empty($request->filename)){
           foreach($request->filename as $one)
           {
               if (isset(explode('/', explode(';', explode(',', $one)[0])[0])[1])) {
                    ...
                    $item_image->attachmentable_type = 'App\Models\Post';
                    $item_image->save();
                }
           }
        }
