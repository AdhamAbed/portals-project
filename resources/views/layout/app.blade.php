...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" @if(session()->get('locale') == 'ar') dir="rtl" @endif>

<head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <title>{{ __('cp.PORTALS FOR TRAINING') }}</title>
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" href="{{url(asset('website'))}}/images/cropped-Logo-Portals-01-1-32x32.png" sizes="32x32">
    
    <link rel="icon" href="{{url(asset('website'))}}/images/cropped-Logo-Portals-01-1-192x192.png" sizes="192x192">
    
    <link rel="apple-touch-icon" href="{{url(asset('website'))}}/images/cropped-Logo-Portals-01-1-180x180.png">
    ...
    @if(session()->get('locale') == 'ar')
        ...
        @else
            <link rel="stylesheet" href="{{url(asset('website'))}}/css/style2.css">
        @endif
    @else
...
@if( Request::is('/home') || Request::is('/'))
    {{--@if(route('home') == true)--}}
    @include('partial.website.navbar3')
@else
    @include('partial.website.navbar4')
@endif
...
{{--                </button>--}}
                ...
                <form method="post" id="handleAjax" action="{{url('do-login')}}" name="postform" class="w-100" enctype="multipart/form-data">
                    @csrf
                    <div class="position-relative w-100">
                        <input  class="moadl-btn  @error('email') is-invalid @enderror" style="    padding-left: 50px;" type="text" name="email" id="" placeholder="email">
                ...
                <span style="margin-top: 16px;">or <a href=""  data-bs-toggle="modal" data-bs-target="#forget" style="color: #59CCC7;">Forget Password</a></span>
                <hr class="w-100" style="border-top: 1px solid #E2E2E2;">
                <span>Don't have an account? <a href="" style="color: #59CCC7;" data-bs-toggle="modal" data-bs-target="#SignUp">SignUp</a></span>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body d-flex flex-column align-items-center justify-content-center">
                ...
                <form method="post" id="registerUser" action="{{url('registerUser')}}" class="w-100" enctype="multipart/form-data">
                    @csrf
                    <div class="position-relative w-100">
                        <input  class="moadl-btn  @error('name') is-invalid @enderror" style="padding-left: 15px"  type="text" name="name" id="" placeholder="Full Name">

                    </div>
                    <div class="position-relative w-100">
                        <input  class="moadl-btn @error('email') is-invalid @enderror" style="padding-left: 15px" type="email" name="email" id="" placeholder="Email">

                    </div>

                    <div class="position-relative w-100">
                        <input  class="moadl-btn @error('password') is-invalid @enderror" style="padding-left: 15px"  type="password" name="password" id="" placeholder="Password">

                    </div>
                    <div class="position-relative w-100">
                        <input  class="moadl-btn @error('confirm_password') is-invalid @enderror" style="padding-left: 15px"  type="password" name="confirm_password" id="" placeholder="Confirm Password">
                    ...
                    <button type="submit" class="btn-filled w-100" style="border-radius: 8px; height: 55px;">Checkout</button>
                </form>
...
<div class="modal fade" id="forget" tabindex="-1" role="dialog" aria-labelledby="forgetTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body d-flex flex-column align-items-center justify-content-center">
                <form action="" class="w-100">
                    <div class="position-relative w-100">
                        <input  class="moadl-btn" type="text" name="" id="" placeholder="email">
                        <svg class="input-icon" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g clip-path="url(#clip0_874_3152)">
                                <path d="M15.5742 3.20837H6.41667C4.39162 3.20837 2.75 4.85 2.75 6.87504V15.125C2.75 17.1501 4.39162 18.7917 6.41667 18.7917H15.5742C17.5992 18.7917 19.2408 17.1501 19.2408 15.125V6.87504C19.2408 4.85 17.5992 3.20837 15.5742 3.20837Z" stroke="#59CCC7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                <path d="M2.75 9.61584C2.75 10.1933 3.0525 10.725 3.5475 11.0183L9.72582 14.7033C10.505 15.1708 11.495 15.1708 12.2742 14.7033L18.4525 11.0183C18.9475 10.725 19.25 10.1933 19.25 9.61584" stroke="#59CCC7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    ...
                    <button class="btn-filled w-100" style="border-radius: 8px; height: 55px; margin: 5px 0;">Reset Password</button>
                </form>

                <hr class="w-100" style="border-top: 1px solid #E2E2E2;">
                <span>Or <a href="" style="color: #59CCC7;" data-bs-toggle="modal" data-bs-target="#SignIn">log in</a></span>
...
<!--- Start Footer --->
@include('partial.website.footer3')
...
<script type="text/javascript">
    var url = "{{ route('website.LangChange') }}";
    const asset_url = '{{url(asset('website'))}}';

    $(".Langchange").change(function(){
        window.location.href = url + "?lang="+ $(this).val();

        console.log(url + "?lang="+ $(this).val(),'ssssssssssss')
    });

    $('a[data-notif-id]').click(function () {
        var csrf_token = $('meta[name="csrf-token"]').attr('content');

        var notification_id   = $(this).data('notifId');
        var targetHref = $(this).data('href');
        // console.log(targetHref ,notification_id , 'dddddddddd');
        {{--$.post('{{ route('notifMarkAsRead') }}', {'notification_id': notification_id, '_token': '{{ csrf_token() }}'}, function (data) {--}}
        {{--    data.success ? (window.location.href = targetHref) : false;--}}
        {{--}, 'json');--}}
        $.ajax({
            url: '{{ route('notifMarkAsRead') }}',
            type: 'POST',
            data: {
            ...
            success: function (data) {
                if (data.success) {
                    // var actionUrl = data.notification.data.action_url;
...
<script>
    $(function() {
        ...
        $(document).on("submit", "#registerUser", function() {
            var e = this;
            // change login button text before ajax
            ...
            $.post($(this).attr('action'), $(this).serialize(), function(data) {
                ...
                for (var err in erroJson) {
                    for (var errstr of erroJson[err])
                        $("#register-errors-list").append("<div class='alert alert-danger'>" + errstr + "</div>");
                }
