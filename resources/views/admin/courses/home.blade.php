...
@section('content')
    ...
    <div class="portlet light bordered">
        ...
        @if(has_permission('admin.courses.create') == true)
                        <a href="{{ route('admin.courses.create') }}" style="margin-right: 5px" class="btn btn-primary">
                            {{ __('cp.add') }}
            ...
            <table class="table table-striped table-bordered table-hover table-checkable order-column" id="toolsTable">
                ...
                @foreach($items as $one)
                  ...
                  @if(has_permission('admin.courses.show') == true)
                        <a href="{{ route('admin.courses.show', $one->id) }}" class="btn btn-xs tooltips"
                      ...
                      @if(has_permission('admin.courses.show') == true)
                          <a href="{{ route('admin.units.index', ['id' => $one->id]) }}" title="{{ __('cp.units') }}" class="btn btn-xs tooltips"
                             data-container="body" data-placement="top" data-original-title="{{ __('cp.units') }}">
                              <i class="far fa-file-alt"></i>
                          </a>
                      @endif
                      @if(has_permission('admin.courses.show') == true)
                          <a href="{{ route('admin.course_statistics.index', ['id' => $one->id]) }}" title="{{ __('cp.statistics') }}" class="btn btn-xs tooltips"
                             data-container="body" data-placement="top" data-original-title="{{ __('cp.statistics') }}">
                              <i class="fa fa-calculator"></i>
                          </a>
                      @endif

                        @if(has_permission('admin.courses.edit') == true)
                        <a href="{{ route('admin.courses.edit', $one->id) }}" class="btn btn-xs tooltips"
