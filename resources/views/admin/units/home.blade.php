            ...
            <div class="table-toolbar">
                <div class="row">
                    ...
                    @if(has_permission('admin.courses.create') == true)
                        <a href="{{ route('admin.units.create', ['id' => $course->id]) }}" style="margin-right: 5px" class="btn btn-primary">
                            {{ __('cp.add') }}
            ...
            <table class="table table-striped table-bordered table-hover table-checkable order-column" id="toolsTable">
                ...
                @foreach($items as $one)
                    <tr class="odd gradeX" id="tr-{{$one->id}}">
                        ...
                        <!--@if(has_permission('admin.units.show') == true)-->
                        <!--<a href="{{ route('admin.units.show', $one->id) }}" class="btn btn-xs tooltips"-->
...
{{--                        @if(has_permission('admin.courses.show') == true)--}}
{{--                        <a href="{{ route('admin.lessons.createMeeting', ['unit_id' => $one->id]) }}" title="{{ __('cp.lessons') }}" class="btn btn-xs tooltips"--}}
{{--                           data-container="body" data-placement="top" data-original-title="{{ __('cp.lessons') }}">--}}
{{--                            <i class="far fa-file"></i>--}}
{{--                        </a>--}}
{{--                        @endif--}}

                        @if(has_permission('admin.courses.show') == true)
                        <a href="{{ route('admin.lessons.index', ['id' => $one->id]) }}" title="{{ __('cp.lessons') }}" class="btn btn-xs tooltips"
                           data-container="body" data-placement="top" data-original-title="{{ __('cp.lessons') }}">
...
{{--                        @if(has_permission('admin.courses.show') == true)--}}
{{--                        <a href="{{ route('admin.exams.index', ['id' => $one->id]) }}" title="{{ __('cp.exams') }}" class="btn btn-xs tooltips"--}}
{{--                           data-container="body" data-placement="top" data-original-title="{{ __('cp.exams') }}">--}}
...
{{--                        @endif--}}
                        ...
                        @if(has_permission('admin.courses.edit') == true)
                        <a href="{{ route('admin.units.edit', $one->id) }}" class="btn btn-xs tooltips"
